...
namespace FactionColonies
{
    public class MilitaryJobHandler_Raid : MilitaryJobHandler
    {
        ...
        public override BattleResult OnResolved(WorldObjectComp_SettlementMilitary milComp)
        {
            ...
            {
                ApplyVictoryToTarget(faction, milComp.WorldSettlement, milComp.militaryEnemy, target);
            }
            else
            {
        ...
        /* Shared victory side effects: loot, prisoners, XP, delivery event. Used by both the
         * legacy comp-driven OnResolved and the op-aware ApplyResult so the two paths produce
         * identical outcomes during the Phase 2 transition window. */
